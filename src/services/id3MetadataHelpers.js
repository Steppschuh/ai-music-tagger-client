...
exports.iso6391To6392 = iso6391To6392;
exports.camelotToStandardKey = camelotToStandardKey;
exports.formatMultipleValues = formatMultipleValues;
exports.createTXXXFields = createTXXXFields;
exports.transformAnalysisToMetadata = transformAnalysisToMetadata;
exports.isFieldEmpty = isFieldEmpty;
exports.mergeMetadata = mergeMetadata;
exports.prepareMetadataForWriting = prepareMetadataForWriting;
exports.normalizeMetadataForComparison = normalizeMetadataForComparison;
exports.formatMetadataForLog = formatMetadataForLog;
/**
 ...
 */
function camelotToStandardKey(camelotKey) {
    const mapping = {
        "1A": "Abm",
        "1B": "B",
        "2A": "Ebm",
 ...
 */
function addTXXXField(array, description, value) {
 ...
 */
function createTXXXFields(analysis) {
    const txxx = [];
    // Energy fields
    if (analysis.moodsAndFeelings?.energy) {
        ...
        if (analysis.moodsAndFeelings.energyScore !== undefined) {
            addTXXXField(txxx, "Energy Score", analysis.moodsAndFeelings.energyScore.toFixed(2));
        }
    }
    // Danceability fields
    if (analysis.moodsAndFeelings?.danceability) {
        ...
        if (analysis.moodsAndFeelings.danceabilityScore !== undefined) {
            addTXXXField(txxx, "Danceability Score", analysis.moodsAndFeelings.danceabilityScore.toFixed(2));
        }
    }
    // Valence
    ...
    if (analysis.mixing?.mixabilityScore !== undefined) {
        addTXXXField(txxx, "Mixability Score", analysis.mixing.mixabilityScore.toFixed(2));
    }
    if (analysis.mixing?.commercialAppealScore !== undefined) {
        addTXXXField(txxx, "Commercial Appeal Score", analysis.mixing.commercialAppealScore.toFixed(2));
    }
    // Mix roles and occasions
    if (analysis.mixing?.suitableMixRoles?.length > 0) {
        addTXXXField(txxx, "Mix Roles", analysis.mixing.suitableMixRoles.join(", "));
    }
    if (analysis.mixing?.suitableOccasions?.length > 0) {
        addTXXXField(txxx, "Occasions", analysis.mixing.suitableOccasions.join(", "));
    }
    // Mixing notes
    ...
    if (analysis.genres?.secondary?.length > 0) {
        addTXXXField(txxx, "Secondary Genres", analysis.genres.secondary.join(", "));
    }
    ...
    if (analysis.moodsAndFeelings?.emotions?.length > 0) {
        addTXXXField(txxx, "Emotions", analysis.moodsAndFeelings.emotions.join(", "));
    }
    // Instrumentation
    if (analysis.instrumentation?.instrumentationType) {
        addTXXXField(txxx, "Instrumentation Type", analysis.instrumentation.instrumentationType);
    }
    if (analysis.instrumentation?.leadInstruments?.length > 0) {
        addTXXXField(txxx, "Lead Instruments", analysis.instrumentation.leadInstruments.join(", "));
    }
    // Vocals
    ...
    if (analysis.vocals?.lyricalThemes?.length > 0) {
        addTXXXField(txxx, "Lyrical Themes", analysis.vocals.lyricalThemes.join(", "));
    }
    if (analysis.vocals?.explicitContent !== undefined) {
        addTXXXField(txxx, "Explicit Content", analysis.vocals.explicitContent.toString());
    }
    ...
    if (analysis.vocals?.sampleSources?.length > 0) {
        addTXXXField(txxx, "Sample Sources", analysis.vocals.sampleSources.join(", "));
    }
    ...
    if (analysis.compositionAndProduction?.rhythmicComplexityLevel) {
        addTXXXField(txxx, "Rhythmic Complexity", analysis.compositionAndProduction.rhythmicComplexityLevel);
    }
    if (analysis.compositionAndProduction?.rhythmicFeel) {
        addTXXXField(txxx, "Rhythmic Feel", analysis.compositionAndProduction.rhythmicFeel);
    }
    if (analysis.compositionAndProduction?.melodicComplexityLevel) {
        addTXXXField(txxx, "Melodic Complexity", analysis.compositionAndProduction.melodicComplexityLevel);
    }
    if (analysis.compositionAndProduction?.harmonicSystem) {
        addTXXXField(txxx, "Harmonic System", analysis.compositionAndProduction.harmonicSystem);
    }
    if (analysis.compositionAndProduction?.harmonicComplexityLevel) {
        addTXXXField(txxx, "Harmonic Complexity", analysis.compositionAndProduction.harmonicComplexityLevel);
    }
    if (analysis.compositionAndProduction?.productionStyle) {
        addTXXXField(txxx, "Production Style", analysis.compositionAndProduction.productionStyle);
    }
    if (analysis.compositionAndProduction?.timbreDescriptors?.length > 0) {
        addTXXXField(txxx, "Timbre Descriptors", analysis.compositionAndProduction.timbreDescriptors.join(", "));
    }
    if (analysis.compositionAndProduction?.sonicTexture?.length > 0) {
        addTXXXField(txxx, "Sonic Texture", analysis.compositionAndProduction.sonicTexture.join(", "));
    }
    ...
    if (analysis.mixing?.suggestedCuePoints?.length > 0) {
        addTXXXField(txxx, "Cue Points", JSON.stringify(analysis.mixing.suggestedCuePoints));
    }
    if (analysis.mixing?.songStructureSegments?.length > 0) {
        addTXXXField(txxx, "Song Structure", JSON.stringify(analysis.mixing.songStructureSegments));
    }
    // Stems analysis (V17)
    if (analysis.stems) {
 ...
 */
function formatTagForHashtag(tag) {
 ...
 */
function collectAllTags(analysis) {
    const tags = new Set();
    // Add primary genres
 ...
 */
function formatTagsAsHashtags(tags) {
    return tags
 ...
 */
function formatTagsAsCSV(tags) {
    return tags.join(", ");
}
/**
 * Generates comment text based on the comment strategy
 */
function generateCommentText(analysis, strategy = "tags+summary") {
    if (strategy === "summary") {
...
function transformAnalysisToMetadata(analysis, commentStrategy = "tags+summary") {
    ...
    if (analysis.vocals?.lyricalLanguages &&
    ...
    if (analysis.mixing?.suitableMixRoles &&
    ...
    if (analysis.vocals?.sampleSources &&
        analysis.vocals.sampleSources.length > 0) {
        // Look for patterns like "Artist - Song" or "Artist Remix"
        const remixInfo = analysis.vocals.sampleSources.find((source) => /remix|mix|edit/i.test(source));
        if (remixInfo) {
...
function isGenreEmpty(value) {
    ...
    if (Array.isArray(value)) {
        const filtered = value.filter((v) => v && typeof v === "string" && v.trim().toLowerCase() !== "music");
 ...
 */
function findTXXXItem(array, description) {
    return array.find((item) => item && item.description !== undefined && item.description === description);
}
/**
 * Sets or updates a TXXX value in an array
 */
function setTXXXValue(array, description, value) {
 ...
 */
function normalizeTXXXArray(txxx) {
    if (!Array.isArray(txxx)) {
        return [];
    }
    return txxx.filter((item) => item &&
 ...
 */
function mergeArrays(existing, newArray) {
 ...
 */
function mergeTXXXArrays(existing, newArray, strategy = "keep-existing") {
...
function mergeMetadata(existing, newMetadata, strategy = "keep-existing") {
    ...
    const mergeField = (field, newValue, isArray = false) => {
        if (!newValue)
            return;
        if (strategy === "overwrite") {
 ...
 */
function prepareMetadataForWriting(existingTags, newMetadata, strategy = "keep-existing") {
...
function normalizeMetadataForComparison(metadata) {
    ...
    if (normalized.userDefinedText && Array.isArray(normalized.userDefinedText)) {
        normalized.userDefinedText = [...normalized.userDefinedText].sort((a, b) => {
    ...
    // Note: genre, mood, language, and contentGroup are now single strings, not arrays
    const arrayFields = []; // No array fields for standard ID3 frames anymore
 ...
 */
function formatMetadataForLog(tags) {
    if (!tags)
        return tags;
    const tryParseJSON = (str) => {
        if (typeof str !== "string" || !str.trim().match(/^(\{|\[)/))
            return null;
        try {
    ...
    const formatImageBuffer = (imgBuf) => {
    ...
    const formatValue = (val, maxLen = 200) => {
    ...
    const formatTXXX = (items) => items.map((item) => {
    ...
    const formatUserDefinedText = (value) => {
        if (Array.isArray(value)) {
            return formatTXXX(value.filter((item) => item?.description && item.description !== ""));
        }
