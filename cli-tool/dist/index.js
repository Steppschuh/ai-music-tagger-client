...
import { formatMetadataForLog, } from "./id3MetadataHelpers.js";
 ...
 */
async function processAudioFile(audioPath, args, tagger) {
    if (args["update-metadata"]) {
        ...
        const commentStrategy = args.commentStrategy;
        const newMetadata = tagger.transformAnalysisToMetadata(analysis, commentStrategy);
        // Merge existing and new metadata
        const mergeStrategy = args.mergeStrategy;
        const mergedMetadata = tagger.mergeMetadata(existingMetadata, newMetadata, mergeStrategy);
