...
export class MusicTaggerClient {
  async analyzeSong(audioPath: string, prompt?: string): Promise<any> {
    ...
    try {
      ...
      console.log(
        `Audio file size: ${(audioFileSize / (1024 * 1024)).toFixed(2)} MB`
      );
      if (audioFileSize > maxAudioFileSize) {
        throw new Error(
          `Audio file size (${audioFileSize}) exceeds the maximum allowed size (${maxAudioFileSize})`
        );
      }
  ...
  async writeAnalysisToFile(audioPath: string, analysis: any): Promise<any> {
    const parsedAudioPath = path.parse(audioPath);
    const analysisFilename = path.join(
      parsedAudioPath.dir,
      `${parsedAudioPath.name}-analysis.json`
    );
  ...
  async readMetadata(audioPath: string): Promise<any> {
    try {
